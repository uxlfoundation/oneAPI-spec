...
``oneapi::tbb::concurrent_unordered_multiset`` is a class template that represents an unordered sequence of elements,
...
``oneapi::tbb::concurrent_unordered_multiset`` is an unordered sequence, which elements are organized into
...
``concurrent_unordered_multiset::key_equal`` is defined as the value of this qualified-id.
...
Otherwise, the member type ``concurrent_unordered_multiset::key_equal`` is defined as the value of the
...
on ``oneapi::tbb::concurrent_unordered_multiset`` objects.
...
in the same internal namespace and define ``oneapi::tbb::concurrent_unordered_multiset`` as a type alias for which
