...
The following tables provides additional information on the members of the
``concurrent_unordered_set`` and
``concurrent_unordered_multiset`` template classes.
...
\ ``std::pair<iterator, bool> insert(const value_type& x)``
  \
  Constructs a copy of ``x`` and attempts to insert it into the
  set. If the attempt fails because an item with the same key already exists, the copy is destroyed.
  ...
  **Returns**: ``std::pair(iterator,success)``. The
  ...
  .. note::
     ...
     This and other methods always succeed for ``concurrent_unordered_multiset``,
...
\ ``iterator insert(const_iterator hint, const value_type& x)``
  ...
  .. note::
     ...
     similarity with ``std::unordered_set``/``std::unordered_multiset``,
...
\ ``std::pair<iterator, bool> insert(value_type&& x)``
  ...
  to insert it into the set. If the attempt fails because an item with the same key
...
\ ``void insert(std::initializer_list<value_type> il)``
  \
  Supported since C++11. Inserts each element from the initializer list into the set.
------------------------------------------------------------------------------------------
\ ``std::pair<iterator, bool> insert(node_type&& nh)``
  \
  Supported since C++11. Attempts to insert the item owned by ``nh``
  into the set. If the attempt fails because an item with the same key already exists,
...
\ ``template<typename... Args> std::pair<iterator, bool> emplace(Args&&... args);``
  ...
  set. If the attempt fails because an item with the same key already exists, this instance is destroyed.
...
\ ``template<typename... Args> iterator emplace_hint(const_iterator hint, Args&&... args);``
  ...
  **Returns**: Iterator pointing to inserted item, or item already in the set
  with the same key.
------------------------------------------------------------------------------------------
\ ``merge``
  \
  ``template<typename SrcHash, typename SrcEquality> void merge
  (concurrent_unordered_multiset<Key, T, SrcHash, SrcEquality, Allocator>& source);``
  
  ``template<typename SrcHash, typename SrcEquality> void merge
  (concurrent_unordered_multiset<Key, T, SrcHash, SrcEquality, Allocator>&& source);``
  
  ``template<typename SrcHash, typename SrcEquality> void merge
  (concurrent_unordered_set<Key, T, SrcHash, SrcEquality, Allocator>& source);``
...
\ ``iterator unsafe_erase(const_iterator position)``
  ...
  **Returns**: Iterator pointing to item that was immediately after the erased
  item, or ``end()`` if erased item was the last item in the set.
------------------------------------------------------------------------------------------
...
\ ``node_type unsafe_extract(const_iterator position)``
  \
  Transfers ownership of the item pointed to by ``position`` from the set
  to a node handle.
...
\ ``node_type unsafe_extract(const key_type& k)``
  \
  If an item with the key equal to ``k`` exists in the set,
...
The following table provides additional information on the
``concurrent_unordered_multiset`` template class.
