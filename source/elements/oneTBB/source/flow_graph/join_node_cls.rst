...
* The ``Bi`` types must meet the :doc:`JoinNodeFunctionObject requirements <../named_requirements/flow_graph/join_node_func_obj>`.
  Alternatively, since C++17 a pointer to const member function in ``Input`` returning ``Key`` or
  a pointer to data member of type ``Key`` in ``Input`` might be used as ``Bi``.
