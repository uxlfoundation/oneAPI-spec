...
* The type ``Body`` must meet the :doc:`FunctionNodeBody requirements <../named_requirements/flow_graph/function_node_body>`.
  Alternatively, since C++17 a pointer to const member function in ``Input`` returning ``Output`` or
  a pointer to data member in ``Input`` of type ``Output`` might be used as a ``Body``.
