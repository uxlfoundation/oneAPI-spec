...
``no_priority``, also the default value of the parameter, switches priority off for a particular node.
...
graph which are available for execution.
...
Consider executing the graph from the picture above using two threads. Let the nodes ``f1`` and
