...
public:
    ...
    ///
    /// @param start The first index in the range. The value shall be greater than or equal to 0.
    /// @param end   The relative end index in the range. Indicates the next index after the
    ...
    ///              For example, :expr:`start=1` and :expr:`end=-2` specify the range of elements
    ...
    /// The relative index of the last element in the range.
    /// If positive, points to the next element after the last one in the range.
    /// If negative, points to the last element in the range.
    std::int64_t end_idx;
};
