...
There are two models of memory management in the SDK implementation: internal
and external.
...
requested memory type and returns :cpp:enumerator:`mfxStatus::MFX_ERR_UNSUPPORTED` for all
other types. The allocation request uses flags, part of the memory type field, to
indicate which SDK class initiated the request, so the external frame allocator
...
the mfxFrameSurface1::mfxFrameSurfaceInterface, if present, or call external
