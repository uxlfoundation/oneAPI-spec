...
The application can configure AVC encoder to work in CQP rate control mode with
HRD model parameters. SDK will place HRD information to SPS/VUI and choose the
...
and QPB). The SDK will choose CBR or VBR HRD mode based on the ``MaxKbps``
parameter. If ``MaxKbps`` is set to zero, the SDK will use CBR HRD model
(write cbr_flag = 1 to VUI), otherwise the VBR model will be used
(and cbr_flag = 0 is written to VUI).

.. note:: For CQP, if implementation doesn't support individual QPI, QPP and QPB
